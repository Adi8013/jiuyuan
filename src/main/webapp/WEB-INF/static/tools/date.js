...
/** 
 * 获取以指定日期（yyyy-MM-dd）为基础的偏移日期，与getPointDate方法不同，该方法可以按指定天数偏移，也可以按指定日月年的差量（为正数时往后偏移，为负数时往前偏移）。<br/>
 * 按指定天数偏移,格式=(-)xx | '(-)xx'：<br/>
 * 		日：<br/>
 * 		getPointDate2('2017-04-05', '20') 或 getDate('2017-04-05', 20) 得到 '2017-04-25';<br/>
 *		getPointDate2('2017-04-05', '-20') 或 getDate('2017-04-05', -20) 得到 '2017-03-16';<br/>
 * 按指日月年的差量偏移，格式='(-)dd (-)mm (-)yy'：<br/>
 *		格式说明：偏移量为一个字符串，以空格隔开分别代表，第一位为 日差（可为负数），第二位为 月差（可为负数），第三位为 年差（可为负数）<br/>
 * 		日月：<br/>
 * 		getPointDate2('2017-04-05', '20 1') 得到 '2017-05-25';<br/>
 *		getPointDate2('2017-04-05', '-20 -1') 得到 '2017-02-13';<br/>
 * 		日月年：<br/>
 * 		getPointDate2('2017-04-05', '20 1 1') 得到 '2018-05-25';<br/>
 *		getPointDate2('2017-04-05', '-20 -1 -1') 得到 '2016-02-14';<br/>
 * @param date 当前日期 string类型；
 * @param offset 偏移量 int类型 或 string类型（分隔符为空格），为正数时往后偏移，为负数时往前偏移；
 * @param separator string类型，date的年月日分隔符，默认为'-'。
 * @return 新的日期，该日期等于当前日期偏移了offsetDay天的日期，格式 YYYY-MM-DD（分隔符以用户定义得分隔符一致，默认'-'）。
 */
function getPointDate2(date, offset, separator){
	var spt = separator || '-';//分隔符为用户指定分隔符，默认'-'
	var dateArry = date.split(spt);
	//默认按天偏移
	var offsetDay = 0;//日差值
	var offsetMonth = 0;//月差值
	var offsetYear = 0;//年差值
	
	if(typeof offset === 'string'){//string类型
		//以空格分割字符串
		var offsetArry = offset.split(' ');
		if(offsetArry.length > 0){//日差
			offsetDay = parseInt(offsetArry[0]);
		}
		if(offsetArry.length > 1){//月差
			offsetMonth = parseInt(parseInt(offsetArry[1])%12);//月差
			offsetYear = parseInt(parseInt(offsetArry[1])/12);//年差
		}
		if(offsetArry.length > 2){//年差
			offsetYear += parseInt(offsetArry[2]);//年差
		}
	}else if(typeof offset === 'number'){//int类型
		offsetDay = offset;
	}else {
		return null;//偏移量类型不对，支持number(int)、string
	}
	//新的日期
	var newDate = new Date(new Date(parseInt(dateArry[0])+offsetYear, parseInt(dateArry[1])-1+offsetMonth, parseInt(dateArry[2])).getTime() + offsetDay*86400000);
	//格式化返回日期 
	return getDateFormat(newDate,'yyyy'+spt+'MM'+spt+'dd');
}
 ...
 */
 ...
 function getMonthDiffNew(strDate1,strDate2){
    ...
    //李杏提出2017-08-03~217-09-02也是完整一个月，故修改
    else if((d2<d1) && (m2) != 2 && (d2 != getDays(strDate2)) && (d2+1!=d1)){
      month = month - 1;
    }
  ...
  /**
  * 获取两个日期的天数差，与getDateDiff方法不同，该方法不需指定多少天为1个月。
  * @param beginDate 起始日期
  * @param endDate 结束日期
  * @param separator 分隔符
  * @return 两个日期的天数差，正数代表结束日期大于起始日期；负数代表结束日期小于起始日期；0代表结束日期等于起始日期。 
  */
  function getDateDiff2(beginDate,endDate, separator){
	  var beginDateArry = beginDate.split(separator || '-');
	  var endDateArry = endDate.split(separator || '-');
	  return (new Date(endDateArry[0], endDateArry[1]-1, endDateArry[2])-new Date(beginDateArry[0], beginDateArry[1]-1, beginDateArry[2]))/86400000;
  }
